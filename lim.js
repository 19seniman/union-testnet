...
async function pollPacketHash(txHash, retries = 50, intervalMs = 5000) {
    ...
    return null; // Return null if packet hash not found after retries
}
    ...
    try {
        ...
        for (let i = 1; i <= maxTransaction; i++) {
            ...
            try {
                ...
                // Ensure baseFeePerGas exists, otherwise fallback
                ...
                try {
                    ...
                    continue; // Continue to next transaction even if confirmation fails
                }
                ...
                if (packetHash) {
                    ...
                    logger.warn(`${timelog()} | Wallet ${walletInfo.index} | Failed to get packet hash for transaction: ${tx.hash}`);
                }
                console.log('');
            } catch (err) {
            ...
            if (i < maxTransaction) {
                await delay(1000); // Small delay between transactions for the same wallet
            }
        }
    } catch (err) {
...
/**
 * Runs a single automation cycle, including loading private keys and performing transactions.
 */
async function runAutomationCycle() {
    logger.section(`Starting new automation cycle at ${timelog()}`);
    ...
    if (privateKeys.length === 0) {
        logger.error('No valid private keys found in .env file. Skipping this cycle.');
        return; // Skip the cycle if no keys are found
    }

    if (!globalPair || !globalMaxTransaction || !globalAmount) {
        logger.error('Automation parameters not set. Please restart the script and provide input. Skipping cycle.');
        return;
    }

    for (const walletInfo of privateKeys) {
        // Ensure that globalChain is correctly set based on globalPair for each wallet processing
        let currentChain;
        if (globalPair === 'SEPOLIA_TO_HOLESKY') currentChain = 'SEPOLIA';
        else if (globalPair === 'HOLESKY_TO_SEPOLIA') currentChain = 'HOLESKY';
        else if (globalPair === 'CORN_TO_SEI') currentChain = 'CORN';
        else if (globalPair === 'BSC_TO_SEI') currentChain = 'BSC';
        else {
            logger.error(`Invalid globalPair: ${globalPair}. Skipping wallet ${walletInfo.index}.`);
            continue;
        }

        logger.loading(`Processing Wallet ${walletInfo.index} for ${globalPair}`);
        await sendFromWallet(walletInfo, globalMaxTransaction, currentChain, walletInfo.privateKey, globalAmount, globalPair);
    }
...
/**
 * Handles initial user input and sets up the 24-hour automation loop.
 */
async function startInteractiveSession() {
    header();

    // Initial questions for user input
    while (true) {
        ...
        if (!isNaN(option) && [1, 2, 3, 4].includes(option)) {
            globalPair = option === 1 ? 'SEPOLIA_TO_HOLESKY' :
                         option === 2 ? 'HOLESKY_TO_SEPOLIA' :
                         option === 3 ? 'CORN_TO_SEI' :
                         'BSC_TO_SEI';
            globalChain = option === 1 ? 'SEPOLIA' :
                          option === 2 ? 'HOLESKY' :
                          option === 3 ? 'CORN' :
                          'BSC';
            break;
    ...
    while (true) {
        ...
        globalMaxTransaction = parseInt(maxTransactionInput.trim());
        if (!isNaN(globalMaxTransaction) && globalMaxTransaction > 0) break;
        logger.error('Invalid number. Please enter a positive number.');
    }

    const ticker = CONFIG[globalChain].BASE_TOKEN_NAME;
    while (true) {
        const amountInput = await askQuestion(`Enter ${ticker} amount (e.g., 0.001): `);
        globalAmount = parseFloat(amountInput.trim());
        if (!isNaN(globalAmount) && globalAmount > 0) break;
        logger.error('Amount must be greater than 0.');
    }

    rl.close(); // Close readline after getting all initial inputs

    // Run the first cycle immediately
    logger.info(`Starting first automation cycle with parameters: Pair=${globalPair}, Transactions/Wallet=${globalMaxTransaction}, Amount=${globalAmount} ${ticker}`);
    await runAutomationCycle();

    // Schedule subsequent cycles
    const twentyFourHours = 24 * 60 * 60 * 1000;
    logger.info(`Next automation cycle scheduled for ${moment().tz('Asia/Jakarta').add(24, 'hours').format('HH:mm:ss | DD-MM-YYYY')}`);
    setInterval(async () => {
        await runAutomationCycle();
        logger.info(`Next automation cycle scheduled for ${moment().tz('Asia/Jakarta').add(24, 'hours').format('HH:mm:ss | DD-MM-YYYY')}`);
    }, twentyFourHours);
...
startInteractiveSession().catch((err) => {
    logger.error(`Initial setup error: ${err.message}`);
    process.exit(1); // Exit if initial setup fails
});

// Add a graceful shutdown for Ctrl+C
process.on('SIGINT', () => {
